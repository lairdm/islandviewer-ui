...
{% comment %}Genes for linear view{% endcomment %}
	{ trackName: "{{ plotName|default:"circular" }}Genes",
	  ...
	  items: [
		{% for gene in genes %}
		  {id: {{ gene.id }}, start: {{ gene.start }}, end: {{ gene.end }}, strand: {{ gene.strand }}, name: "{% if gene.gene == None %}{{ gene.locus|default:"Unknown" }}{% else %}{{ gene.gene }}{% endif %}", accnum: "{{ gene.name }}" },
		{% endfor %}
		]
	},
...
//    console.log(second_genomesize);
    ...
    var secondlayout = {genomesize: second_genomesize, container: "#rightplot", h: 500, w: 500, ExtraWidthX: 55, TranslateX: 25, ExtraWidthY: 40, TranslateY: 20, movecursor: true, plotid: 'circularchart', dblclick: '{{ varName|default:'' }}islandviewerObj' };
